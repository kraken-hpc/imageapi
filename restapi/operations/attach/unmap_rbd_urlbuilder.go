...
func (o *UnmapRbdURL) Build() (*url.URL, error) {
	...
	var _path = "/attach/rbd/{id}"

	id := swag.FormatInt64(o.ID)
	if id != "" {
		_path = strings.Replace(_path, "{id}", id, -1)
	} else {
		return nil, errors.New("id is required on UnmapRbdURL")
	}
...
func (o *UnmapRbdURL) BuildFull(scheme, host string) (*url.URL, error) {
	if scheme == "" {
		return nil, errors.New("scheme is required for a full url on UnmapRbdURL")
	}
	if host == "" {
		return nil, errors.New("host is required for a full url on UnmapRbdURL")
	}
