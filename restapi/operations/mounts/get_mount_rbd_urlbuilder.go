...
func (o *GetMountRbdURL) Build() (*url.URL, error) {
	...
	var _path = "/mount/rbd/{id}"

	id := swag.FormatInt64(o.ID)
	if id != "" {
		_path = strings.Replace(_path, "{id}", id, -1)
	} else {
		return nil, errors.New("id is required on GetMountRbdURL")
	}
...
func (o *GetMountRbdURL) BuildFull(scheme, host string) (*url.URL, error) {
	if scheme == "" {
		return nil, errors.New("scheme is required for a full url on GetMountRbdURL")
	}
	if host == "" {
		return nil, errors.New("host is required for a full url on GetMountRbdURL")
	}
